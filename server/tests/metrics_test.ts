...
import { computeSessionMetrics, computeExamSummary } from "../src/services/metrics.ts";
...
Deno.test("computeSessionMetrics: returns focus ratio from heartbeats", () => {
  ...
  try {
    ...
    assertEquals(Math.round(metrics.focusRatio * 1000), 733, "focus ratio should be ~73.3%");
  } finally {
    closeTestDb(db);
  }
});
...
Deno.test("computeSessionMetrics: returns paste ratio from heartbeats", () => {
  ...
  try {
    const session = findOrCreate(db, "student-1", "exam-1");
    insertHeartbeat(db, session.sessionId, makeHeartbeat({
      input: { typedChars: 100, pastedChars: 200, deletedChars: 10, currentLength: 290 },
    }));

    const metrics = computeSessionMetrics(db, session.sessionId);
    // Total input = 100 + 200 + 10 = 310, paste ratio = 200/310 = 0.645
    assertEquals(Math.round(metrics.pasteRatio * 1000), 645, "paste ratio should be ~64.5%");
  } finally {
    closeTestDb(db);
  }
});
...
Deno.test("computeSessionMetrics: returns total copy and paste counts", () => {
  ...
  try {
    const session = findOrCreate(db, "student-1", "exam-1");
    insertHeartbeat(db, session.sessionId, makeHeartbeat({ copyCount: 3, pasteCount: 5 }));
    insertHeartbeat(db, session.sessionId, makeHeartbeat({ copyCount: 2, pasteCount: 1 }));
...
Deno.test("computeSessionMetrics: returns unmatched paste count from events", () => {
  ...
  try {
    ...
    insertEvents(db, session.sessionId, [
      { type: "paste", timestamp: 1000, hash: "aaa", length: 10, matchedCopyHash: "aaa" },
      { type: "paste", timestamp: 2000, hash: "bbb", length: 200, matchedCopyHash: null },
      { type: "paste", timestamp: 3000, hash: "ccc", length: 5, matchedCopyHash: null },
    ]);

    const metrics = computeSessionMetrics(db, session.sessionId);
    assertEquals(metrics.unmatchedPasteCount, 2, "should count pastes with null matchedCopyHash");
  } finally {
    closeTestDb(db);
  }
});
...
Deno.test("computeSessionMetrics: returns largest paste from stored paste content", () => {
  ...
  try {
    ...
    const paste1: PasteContentRequest = {
      hash: "hash1", content: "short", length: 5, sessionId: session.sessionId, examId: "exam-1", timestamp: 1000,
    };
    const paste2: PasteContentRequest = {
      hash: "hash2", content: "a much longer paste content here", length: 31, sessionId: session.sessionId, examId: "exam-1", timestamp: 2000,
    };
    insertPasteContent(db, paste1);
    insertPasteContent(db, paste2);

    const metrics = computeSessionMetrics(db, session.sessionId);
    assertEquals(metrics.largestPasteLength, 31, "should return largest paste length");
    assertEquals(metrics.largestPasteHash, "hash2", "should return hash of largest paste");
  } finally {
    closeTestDb(db);
  }
});
...
Deno.test("computeSessionMetrics: handles session with no heartbeats", () => {
  ...
  try {
    ...
    assertEquals(metrics.unmatchedPasteCount, 0, "unmatched paste count should be 0");
...
Deno.test("computeExamSummary: returns all sessions for an exam with metrics", () => {
  ...
  try {
    ...
    insertHeartbeat(db, session1.sessionId, makeHeartbeat({
      studentId: "student-1",
      focus: { focusedTimeMs: 50000, unfocusedTimeMs: 10000, blurCount: 1 },
      copyCount: 2, pasteCount: 1,
    }));
    insertHeartbeat(db, session2.sessionId, makeHeartbeat({
      studentId: "student-2",
    ...
    assertEquals(Math.round(s1.focusRatio * 1000), 833, "student-1 focus ratio ~83.3%");
  } finally {
    closeTestDb(db);
  }
});
